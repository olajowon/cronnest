...
system_crontab_file = sys.argv[1]
user_crontab_dir = sys.argv[2]

class ParamsErr(Exception):
    pass

def main():
    data = json.loads(base64.b64decode(sys.argv[3]))
    tab = data.get('tab')
    old_job = data.get('old_job')
    new_job = data.get('new_job')
    row = old_job.get('row')

    if not tab or not old_job or not new_job or row is None:
        raise ParamsErr('参数无效')

    if old_job['enabled'] in ('true', 'false'):
        old_job['enabled'] = True if old_job['enabled'] == 'true' else False

    tabfile = system_crontab_file if tab == 'system' else os.path.join(user_crontab_dir, tab)

    if not os.path.isfile(tabfile):
        raise ParamsErr('Tab文件不存在[%s]' % tabfile)

    cron = CronTab(tabfile=tabfile, user=False) if tab == 'system' else CronTab(tabfile=tabfile)
    if row > len(cron)-1:
        raise ParamsErr('old_job.row=%d没有找到该行' % row)

    job = cron[row]

    if job.enabled != old_job['enabled'] or str(job.slices) != old_job['slices'] or \
        job.command != old_job['command'] or job.comment != old_job['comment'] or \
        (tab=='system' and job.user != old_job['user']):
        raise ParamsErr('old_job 与当前row=%d行Job内容不匹配，可能已发生了变化，请更新old_job数据后重试' % row)

    if new_job['enabled'] in ('true', 'false'):
        new_job['enabled'] = True if new_job['enabled'] == 'true' else False

    if new_job['enabled'] not in (True, False):
        raise ParamsErr('new_job.enabled状态参数无效(true or false)')

    if not CronSlices.is_valid(new_job['slices']):
        raise ParamsErr('new_job.slices调度参数[%s]不合法' % new_job['slices'])

    if not new_job['command']:
        raise ParamsErr('new_job.command命令参数是必须项')

    if not new_job['comment']:
        raise ParamsErr('new_job.comment备注参数是必须项')

    if tab == 'system' and not new_job['user']:
        raise ParamsErr('new_job.user用户参数是必须项')

    if job.enabled != new_job['enabled'] or str(job.slices) != new_job['slices'] or \
        job.command != new_job['command'] or job.comment != new_job['comment'] or \
        (tab=='system' and job.user != new_job['user']):
        job.enable(new_job['enabled'])
        job.setall(new_job['slices'])
        job.set_command(new_job['command'])
        job.set_comment(new_job['comment'])
        if tab == 'system':
            job.user = new_job['user']
        cron.write()

    print(json.dumps(dict(tab=tab, job=dict(new_job.items() + dict(row=row).items()))))
