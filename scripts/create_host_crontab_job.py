...
def main():
    ...
    if new_job['enabled'] not in (True, False):
        raise ParamsErr('job.enabled状态参数无效(true or false)')

    if not CronSlices.is_valid(new_job['slices']):
        raise ParamsErr('job.slices调度参数[%s]不合法' % new_job['slices'])

    if not new_job['command']:
        raise ParamsErr('job.command命令参数是必须项')

    if not new_job['comment']:
        raise ParamsErr('job.comment备注参数是必须项')

    if tab == 'system' and not new_job['user']:
        raise ParamsErr('job.user用户参数是必须项')

    for job in cron.find_command(re.compile(r'^%s$' % new_job['command'])):

        if tab == 'system':
            if str(job.slices) == new_job['slices'] and job.user == new_job['user']:
                raise ParamsErr('%s %s %s job已存在' % (new_job['slices'], new_job['user'], new_job['command']))
        else:
            if str(job.slices) == new_job['slices']:
                raise ParamsErr('%s %s job已存在' % (new_job['slices'], new_job['command']))

    for job in cron.find_comment(new_job['comment']):
        raise ParamsErr('job.comment备注[%s]已存在' % new_job['comment'])

    if tab == 'system':
        job = cron.new(command=new_job['command'], comment=new_job['comment'], user=new_job['user'])
    else:
        job = cron.new(command=new_job['command'], comment=new_job['comment'])
