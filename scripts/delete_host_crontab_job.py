...
def main():
    ...
    if not tab or not rm_job or row is None:
        raise ParamsErr('参数无效')

    if rm_job['enabled'] in ('true', 'false'):
        rm_job['enabled'] = True if rm_job['enabled'] == 'true' else False
    ...
    if row > len(cron)-1:
        raise ParamsErr('job.row=%d没有找到该行' % row)

    job = cron[row]

    if job.enabled != rm_job['enabled'] or str(job.slices) != rm_job['slices'] or \
        job.command != rm_job['command'] or job.comment != rm_job['comment'] or \
        (tab=='system' and job.user != rm_job['user']):
        raise ParamsErr('job 与当前row=%d行Job内容不匹配，可能已发生了变化，请更新job数据后重试' % row)


    cron.remove(job)
    cron.write()

    print(json.dumps(dict(tab=tab, job=rm_job)))
