...
if __name__ == '__main__':
    ...
    user_tabs = os.listdir(user_crontab_dir)
    for tab in user_tabs:
        if not tab.startswith('#'):
            tabfile = os.path.join(user_crontab_dir, tab)
    ...
    if os.path.isfile(system_crontab_file):
        cron = CronTab(tabfile=system_crontab_file, user=False)
        crontab['system'] = []
        for idx, job in enumerate(cron):
            crontab['system'].append({
                'row': idx,
                'enabled': job.enabled,
                'slices': str(job.slices),
                'user': job.user,
